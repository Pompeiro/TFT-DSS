...
df = pd.DataFrame(
    columns=["Champion", "DPS", "Attack Speed", "Damage", "Range"]
...
for i in range(1, 5, 1):
    list_category_elements.append(
        driver.find_elements_by_xpath('//a[@class="characters-item c%d"]' % i)
    )
...
def stats_gathering(resultslist, flag):
    ...
    for j in range(0, len(championList), 1):
        ...
        result = [0] * (
            len(find(links[j].text, "\n")) + 1
        )  ### how many elements is in the table
        # print(result)
    ...
    return tuple(
        resultslist
    )  #### i dont want it to be mutable to check step1 and step2
