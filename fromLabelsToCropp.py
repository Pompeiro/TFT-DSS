...
for directori in listOfDirectoriesWithXMLFiles:
    pathToDirectoryWithXMLFiles = os.path.join(
        pathToDirectoryWithXMLDirectories, directori
    )
    ...
    for xmlFile in listOfXMLFilesInDirectory:
        ...
        def findSomethingInXMLFormatReturnStandardTypeList(
            tagToFind="name", standardTypeToConvert=str
        ):
        ...
        championNamesFoundInXMLList = findSomethingInXMLFormatReturnStandardTypeList(
            "name", str
        )
        ...
        if withoutL_Mode:
            ...
            print(
                "championNamesFoundInXMLList after deleting L_ : ",
                championNamesFoundInXMLList,
            )

        championXminFoundInXMLList = findSomethingInXMLFormatReturnStandardTypeList(
            "xmin", int
        )
        championYminFoundInXMLList = findSomethingInXMLFormatReturnStandardTypeList(
            "ymin", int
        )
        championXmaxFoundInXMLList = findSomethingInXMLFormatReturnStandardTypeList(
            "xmax", int
        )
        championYmaxFoundInXMLList = findSomethingInXMLFormatReturnStandardTypeList(
            "ymax", int
        )
        ...
        for i in range(0, len(championXmaxFoundInXMLList), 1):
            championWidthList.append(
                championXmaxFoundInXMLList[i] - championXminFoundInXMLList[i]
            )

            championHeightList.append(
                championYmaxFoundInXMLList[i] - championYminFoundInXMLList[i]
            )

            topLeft = (championXminFoundInXMLList[i], championYminFoundInXMLList[i])
            bottomRight = (
                topLeft[0] + championWidthList[i],
                topLeft[1] + championHeightList[i],
            )
            center = (
        ...
        def draw_rectangle_and_center_and_show(
            screenshot,
            markerPosition=markerPositionList[0],
        ...
        def create_directory_with_label_name(
            nameOfDirectory=championNamesFoundInXMLList[0], parentDir=parentDirectory
        ):
            # Directory
        ...
        for i in range(0, len(championNamesFoundInXMLList), 1):
            ...
            screenName = championNamesFoundInXMLList[i] + "{:08}.jpg".format(
                len(os.listdir(tempPath))
            )
