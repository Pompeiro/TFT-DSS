...
def from_OCR_champions_to_buy_list_to_counter_index_list():
    ...
    print(
        "Should be max 5 items there !!!!!!!!!!!!!!! if not check from_OCR_champions_to_buy_list_to_counter_index_list: ",
        counterIndexList,
    )
...
def show_points_for_champions_to_buy():
    ...
    for i in range(0, len(counterIndexListLocal), 1):
        pointsForChampionsToBuy[i] = (
            df.Points[counterIndexListLocal[i]]
            + additional_points_from_origin_combo(counterIndexListLocal[i])
            + additional_points_from_class_combo(counterIndexListLocal[i])
            + additional_points_from_champions_in_pool(counterIndexListLocal[i])
        )
...
def create_list_sorted_champions_to_buy_points_then_indexes_then_position_on_screen(
    pointsForChamp=pointsForChampionsInGameToBuyINITIAL,
    champsTOBUYINDEXES=champsToBuyIndexesINITIAL,
    posONSCREEN=posOnScreenINITIAL,
):
    #### https://stackoverflow.com/questions/6422700/how-to-get-indices-of-a-sorted-array-in-python
    print(
        "inputs to create_list_sorted_champions_to_buy_points_then_indexes_then_position_on_screen()"
    )
    ...
    for i, point in enumerate(pointsForChamp):
        championsToBuyPointsThenIndexesThenPositionOnScreen.append(
            [point, champsTOBUYINDEXES[i], posONSCREEN[i]]
        )

    sorted_inds, sorted_items = zip(
        *sorted(
            [
                (i, e)
    ...
    print(
        "Output create_list_sorted_champions_to_buy_points_then_indexes_then_position_on_screen"
    )
    print("sorted_items")
    print(sorted_items)
...
SORTEDchampionsToBuyPointsThenIndexesThenPositionOnScreen = [
    [1.2645505077078336, 36, 0],  ### hardcoded only for give this initial value
...
def make_cropped_ss(
    ...
    saveMode=0,
    savingName="sss.jpg",
    savingNameUnique="ssIlony",
    saveModeUnique=0,
    parDirectory="C:\\Users\\janusz\\Pictures\\tft\\testingimages\\",
    imageToLoad=None,
):
    WINDOWCAPTUREFLAG = 1
    if loadImage:
    ...
    if WINDOWCAPTUREFLAG or loadImage:
        ...
        if saveModeUnique:
            cv.imwrite(
                unique_file((parDirectory + "\\" + savingNameUnique), "jpg"), crop_img
            )
...
def update_champion_counter(
    champWithPointsThenIndexThenPosition=SORTEDchampionsToBuyPointsThenIndexesThenPositionOnScreen[
        0
    ],
):
    ...
    championsCounterList[champWithPointsThenIndexThenPosition[1]] = (
        championsCounterList[champWithPointsThenIndexThenPosition[1]] + 1
    )
    print(
        "Bought champion: ",
        championListForOCR[champWithPointsThenIndexThenPosition[1]],
...
def buy_best_available_champions_by_points(
    howMuchChampions=2,
    mousePathDelay=0.01,
    inGameWindow=Screenshotwindow,
    sortedChampionsToBuyPoints=SORTEDchampionsToBuyPointsThenIndexesThenPositionOnScreen,
):
    ...
    for i in range(0, howMuchChampions, 1):
        ...
        pyautogui.moveTo(
            x=championToBuyPositionOnGame[sortedChampionsToBuyPoints[i][2]][0],
            y=championToBuyPositionOnGame[sortedChampionsToBuyPoints[i][2]][1],
...
def click_on_buyXPButton_or_refreshButton(buttonToClick=1):
    ...
    if buttonToClick:
        if match_template_with_screen(
            ...
            activate_game_window()
            pyautogui.moveTo(
                x=gameBuyXPButtonXY[0] + gameBuyXPButtonHW[1] // 2,
                y=gameBuyXPButtonXY[1] + gameBuyXPButtonHW[0] // 2,
                duration=0.05,
            )
            pyautogui.mouseDown()
            # time.sleep(0.1)
        ...
        if match_template_with_screen(
            hexesToCheckListJPG=gameRefreshButtonJPG,
            hexesLocationWithOffset=gameRefreshButtonXY,
            HW=gameRefreshButtonHW,
            threshold=0.95,
        ):
            activate_game_window()
            pyautogui.moveTo(
                x=gameRefreshButtonXY[0] + gameRefreshButtonHW[1] // 2,
                y=gameRefreshButtonXY[1] + gameRefreshButtonHW[0] // 2,
                duration=0.05,
            )
            pyautogui.mouseDown()
            # time.sleep(0.1)
...
def buy_champ_if_has_more_points_than_threshold():
    ...
    if sum(i >= THRESHOLDFORPOINTSTOBUYCHAMPION for i in pointsForChampionsInGameToBuy):
        ...
        SORTEDchampionsToBuyPointsThenIndexesThenPositionOnScreen = list(
            create_list_sorted_champions_to_buy_points_then_indexes_then_position_on_screen(
                pointsForChamp=pointsForChampionsInGameToBuy,
                champsTOBUYINDEXES=championsToBuyIndexes,
                posONSCREEN=posOnScreen,
            )
        )

        buy_best_available_champions_by_points_threshold(
            threshold=THRESHOLDFORPOINTSTOBUYCHAMPION,
            inGameWindow=Screenshotwindow,
            sortedChampionsToBuyPoints=SORTEDchampionsToBuyPointsThenIndexesThenPositionOnScreen,
        )
        ...
        make_cropped_ss(
            savingNameUnique="ssIlony", saveModeUnique=1, parDirectory=parentDirectory
        )
...
boost_up_points_for_class(clas='"Duelist"')
        ...
        if checkingRound[1]:
            ...
            try:
                ...
                if roundNow in ROUNDSTOBUYXP:
                    while (
                        click_on_buyXPButton_or_refreshButton(buttonToClick=1) == 1
